...
Since I learned React with Redux a long time ago, I always assumed it was better to use a library, like Formik, to create forms. With you example, it was very refreshing to see how easy is to create form validations with modern React alone.

Feedback:
* For the last credit, you mentioned we shouldn't set `event.target.value` in the `handleChange` because that's imperative. However, the alternative (using `useState`) requires a lot more code which is also a downside. Could you expand more into the downsides of using `event.target.value = `?
* Since we are using a 'Username' field for the exercise, my password manager thinks it needs to autocomple the field, showing a popup.
